...
export default class AIController {
    ...
    constructor() {
        ...
        this.streamContinueConversation = this.streamContinueConversation.bind(this);
    }
    ...
    */
     ...
     */
    async streamChatCompletion(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { messages, maxTokens, temperature } = req.body;
            ...
            const { stream, conversationPromise } = await this.aiService.createStreamingChatCompletion(
                messages,
                userId,
                { maxTokens, temperature }
            );
            ...
            // Wait for conversation to be saved and send the final message with the conversation ID
            const { conversationId, fullResponse } = await conversationPromise;
            res.write(`data: ${JSON.stringify({
                done: true,
                conversationId,
                usage: {
                    promptTokens: this.aiService.estimateTokenCount(messages.map(m => m.content).join(' ')),
                    completionTokens: this.aiService.estimateTokenCount(fullResponse)
                }
            })}\n\n`);

            res.end();
        } catch (err) {
            console.error('Error in streaming chat completion:', err);
     ...
     */
    async streamContinueConversation(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            // Get streaming response
            const { stream, conversationId: confirmedId, savePromise } =
                await this.aiService.continueConversationStream(
                    conversationId,
                    message,
                    userId,
                    { maxTokens, temperature }
            ...
            res.write(`data: ${JSON.stringify({
                ...
                usage: {
                    promptTokens: this.aiService.estimateTokenCount(message),
                    completionTokens: this.aiService.estimateTokenCount(fullResponse)
                }
            })}\n\n`);
            ...
            console.error('Error in streaming conversation continuation:', err);
